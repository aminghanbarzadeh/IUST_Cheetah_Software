...
void ConvexMPCLocomotion::_SetupCommand(ControlFSMData<float> & data){
    ...
    if(data.controlParameters->use_rc){ // remote commander
        ...
        x_vel_cmd = data._desiredStateCommand->leftAnalogStick[1]*0.5;
        y_vel_cmd = data._desiredStateCommand->leftAnalogStick[0]*0.25;
        step_height = data.userParameters->Swing_traj_height;
    }
    _x_vel_des = _x_vel_des*(1-filter) + x_vel_cmd * filter;
    _y_vel_des = _y_vel_des*(1-filter) + y_vel_cmd * filter;
    // printf("yaw turn rate: %f \n", _yaw_turn_rate);
    // printf("yaw es: %f \n", data._stateEstimator->getResult().rpy[2]);
...
void ConvexMPCLocomotion::run(ControlFSMData<float>& data) {
    ...
    if(((gaitNumber == 4) && current_gait != 4) || firstRun)
    {
        ...
        // stand_traj [2] = seResult.position [2];  //////////// Attention: They gave the hard code values for z in here but we are using SE.
...
//    foot postion in world frame
    ...
    //       get real hip location from com
        ...
        Pf[0] +=  pfx_rel;
        Pf[1] +=  pfy_rel;
        Pf[2] = -0.01;              // Attention: what is this?
...
if (data._quadruped->_robotType == RobotType::IUST){
    ...
    float Q_MILAB[12] = {1,     1,    1, 2, 2, 50, 0, 0,  1,    1,  1,  1}; // milab
    alpha = 8e-6; // milab
...
void ConvexMPCLocomotion::initMilabSparseMPC() { //For Milab robot  //////////////// Attention: we don't have the right inertia here
    Mat3<double> baseInertia;
