...
void LegController<T>::updateData(const SpiData* spiData) {
  for (int leg = 0; leg < 4; leg++) {
    ...
    // printf("q joints000000000000000000: %f %f %f\n", spiData->q_abad[0], spiData->q_hip[0], spiData->q_knee[0]);
    // printf("q joints111111111111111111: %f %f %f\n", spiData->q_abad[1], spiData->q_hip[1], spiData->q_knee[1]);
    // printf("q joints222222222222222222: %f %f %f\n", spiData->q_abad[2], spiData->q_hip[2], spiData->q_knee[2]);
    // printf("q joints333333333333333333: %f %f %f\n", spiData->q_abad[3], spiData->q_hip[3], spiData->q_knee[3]);
