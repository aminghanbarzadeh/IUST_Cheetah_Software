...
def main():
    parser = argparse.ArgumentParser(description='Publish Xbox Controller inputs over LCM.')
    parser.add_argument('--lcm-url', type=str, default=None,
                        help='LCM URL to use (e.g., udpm://239.255.76.67:7667?ttl=1). If not provided, it uses the LCM_DEFAULT_URL environment variable or default multicast.')
    ...
    # Initialize LCM
    lcm_url = args.lcm_url or os.environ.get('LCM_DEFAULT_URL', '')
    if lcm_url:
        print(f"Initializing LCM with URL: {lcm_url}")
        lc = lcm.LCM(lcm_url)
    else:
        print("Initializing LCM with default settings.")
        lc = lcm.LCM()
    ...
    try:
        while True:
            ...
            # Print for debugging (optional, can be commented out)
            # print(f"A:{msg.a} B:{msg.b} X:{msg.x} Y:{msg.y} | LTrig:{msg.leftTriggerAnalog:.2f} RTrig:{msg.rightTriggerAnalog:.2f}")
